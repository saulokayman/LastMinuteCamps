...
const BASE_URL = `https://${SUPABASE_PROJECT_ID}.supabase.co/functions/v1/make-server-908ab15a/cron/snapshot?secret=${encodeURIComponent(CRON_SECRET)}`;
...
if (!apiKey) {
  ...
  console.log('');
  console.log('Usage:');
  console.log('  node setup-cron-jobs-download.mjs YOUR_API_KEY_HERE');
  console.log('');
...
async function createCronJob(job) {
  ...
  try {
    ...
    if (!responseText) {
      return { success: false, title: job.title, error: `Empty response with status ${response.status}` };
    }
    ...
    try {
      ...
      return { success: false, title: job.title, error: `Invalid JSON response: ${responseText}` };
    }

    if (response.ok && data.jobId) {
      console.log(`✅ Created successfully! Job ID: ${data.jobId}`);
